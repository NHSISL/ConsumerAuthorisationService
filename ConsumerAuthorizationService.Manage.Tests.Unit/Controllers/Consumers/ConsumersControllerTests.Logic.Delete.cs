...
namespace ConsumerAuthorizationService.Manage.Tests.Unit.Controllers.Consumers
{
    ...
    {
        [Fact]
        public async Task ShouldRemoveConsumerOnDeleteByIdAsync()
        {
            // given
            ...
            consumerServiceMock
                .Setup(service => service.RemoveConsumerByIdAsync(inputId))
                    .ReturnsAsync(storageConsumer);

            // when
            ActionResult<Consumer> actualActionResult =
                await consumersController.DeleteConsumerByIdAsync(inputId);

            // then
            actualActionResult.ShouldBeEquivalentTo(expectedActionResult);

            consumerServiceMock
                .Verify(service => service.RemoveConsumerByIdAsync(inputId),
                    Times.Once);
