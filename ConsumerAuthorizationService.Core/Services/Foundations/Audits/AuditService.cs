    ...
    internal partial class AuditService : IAuditService
    {
        ...
        TryCatch(async () =>
        {
            ...
            Audit audit = new Audit
            {
                ...
                CreatedWhen = dateTimeOffset,
                UpdatedBy = auditUserId ?? string.Empty,
                UpdatedWhen = dateTimeOffset,
            };
        ...
        public ValueTask<Audit> RemoveAuditByIdAsync(Guid auditId, string reason = "") =>
        TryCatch(async () =>
        {
            ValidateAuditId(auditId);
        ...
        virtual internal async ValueTask<List<Audit>> ValidateAuditsAndAssignIdAndAuditAsync(List<Audit> audits)
        {
            ...
            foreach (Audit address in audits)
            {
                try
                {
                    ...
                    address.CreatedWhen = currentDateTime;
                    address.CreatedBy = currentUserId;
                    address.UpdatedWhen = address.CreatedWhen;
